...
const filteredPosts = posts.posts ? posts.posts.filter(post =>
        ...
        {selectedPost ? (
          ...
          <div className={`grid ${viewMode === "grid" ? "grid-cols-3" : "grid-cols-1"} gap-4`}>
            {filteredPosts.map(post => (
              <div key={post._id} className="p-4 border rounded-md cursor-pointer bg-post shadow-md hover:shadow-lg" onClick={() => handlePostClick(post._id)}>
